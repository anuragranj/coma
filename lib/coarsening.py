...
def unperm_data(xnew, indices, M):
    """
...
def perm_tensor(x, indices):
    xnew = np.zeros((x.shape[0], len(indices), x.shape[2]), dtype=x.dtype)
    for i in range(x.shape[2]):
        xnew[:,:,i] = perm_data(x[:,:,i], indices)
    return xnew


def unperm_tensor(xnew, indices, M):
    x = np.zeros((xnew.shape[0], M, xnew.shape[2]), dtype=xnew.dtype)
    for i in range(xnew.shape[2]):
        x[:,:,i] = unperm_data(xnew[:,:,i], indices, M)
    return x
